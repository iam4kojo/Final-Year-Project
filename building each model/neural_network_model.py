...
model_nn = Sequential()
model_nn.add(Dense(128, activation='relu', input_shape=(xTrain.shape[1],)))
...
model_nn.fit(X_train_resampled_scaled, y_train_resampled_scaled, epochs=10, batch_size=32, validation_split=0.1) # Using the processed data resulted in better values for the ROC AUC Score and same values for everything else

# Make predictions on the test data
y_pred_prob_nn = model_nn.predict(xTest)
y_pred_nn = (y_pred_prob_nn > 0.5).astype(int)

# Evaluate the model
from sklearn.metrics import accuracy_score, precision_score, recall_score, f1_score, roc_auc_score
accuracy = accuracy_score(yTest, y_pred_nn)
if any(y_pred_nn == 1):
    precision = precision_score(yTest, y_pred_nn)
else:
    precision = 0.0
recall = recall_score(yTest, y_pred_nn)
f1 = f1_score(yTest, y_pred_nn)
roc_auc = roc_auc_score(yTest, y_pred_prob_nn)
